...
- 中文文本语义相似度计算模块。输入车辆产品设计的feature与scenario文本，计算得到两个文本的相似度，并最终计算该feature与所有scenario的相似度，并结合人工检验方法构建的多路召回与多路输出，筛选与该feature语义接近的所有scenario，支持后续产品设计。
...
数据分为预训练数据与微调数据
- 预训练数据共1647k，由两部分构成：
（1）hugging face上提供的车辆购买、维修相关对话数据wushan_vehicle_qa，我们从中提取了训练数据与测试数据，并将原始数据一并提供；
（2）使用给定feature中的关键词从知乎专栏中搜索获取的文本数据。
该数据仅包含文本，并没有feature与scenario那样的相似度匹配关系，因此该数据无法用于fine-tune，只能用于预训练。为此，我们在这一版代码中补充了预训练的接口。已有研究表明，预训练需要大量的数据与训练资源支持才有较好效果，而受限于行业公开数据资源的限制，这里的预训练代码效果可能不佳，在补充更多数据后可能有更好效果，因此补充的预训练代码模块仅作为参考保留。

- 微调数据由大众业务部门提供，人工匹配的具有语义相关关系的feature和scenario文本对，共677k。

### 更换自己的数据集
 - 按照data目录下数据集的格式构建的数据可以替换使用。  
